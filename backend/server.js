//server setup : handles the request from localhost 5173
  ...
  socket.on("submitAnswer", (room, answerIndex) => {
    ...
    if (currentPlayer) {
      ...
      currentPlayer.score = isCorrect
        ...
        : ((currentPlayer.score === 0 ? 0 : (currentPlayer.score || 0) - 1));
      ...
      if (winner) {
        io.to(room).emit("gameOver", { winner : (winner? winner.name : "No one" )});
        delete rooms[room];
      } else {
        askNewQuestion(room);
      }
